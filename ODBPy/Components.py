...
def read_components(directory):
    ...
    top_path = os.path.join(directory, f"steps/{product_model}/layers/comp_+_top/components")
    top_components = read_linerecords(top_path) if os.path.isfile(top_path) else {}
    bot_path = os.path.join(directory, f"steps/{product_model}/layers/comp_+_bot/component2_skip_for_now")
